...
- Если refresh неуспешен — очистка токенов и возврат на экран входа (`sessionExpired`).
...
Очередь **`outbound_batches`** в Room отправляется **WorkManager** при появлении сети (head-of-line по `id`). После **409** локальная запись удаляется (копия остаётся на сервере в `failed_sync_batches`).
